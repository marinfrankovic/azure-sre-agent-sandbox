...
GRAFANA_PNA="$(azd_value AZURE_GRAFANA_PUBLIC_NETWORK_ACCESS)"
PRIVATE_NET="$(azd_value AZURE_PRIVATE_NETWORKING_ENABLED)"
...
echo "Private networking : $PRIVATE_NET  (Grafana public access: $GRAFANA_PNA)"
echo ""
...
echo "     subscription Owner is NOT sufficient. See README 'Granting access'."if [ "$PRIVATE_NET" = "true" ] && [ "$GRAFANA_PNA" = "Disabled" ]; then
  echo "  4. Grafana is PRIVATE (public access Disabled): reach it from inside the VNet"
  echo "     (Bastion/jumpbox/VPN). If the SRE Agent cannot reach private Grafana,"
  echo "     redeploy with GRAFANA_PUBLIC_NETWORK_ACCESS=Enabled."
fiecho "==============================================================="
